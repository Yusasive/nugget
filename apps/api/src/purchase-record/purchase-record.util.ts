...
export function computePurchaseTotal(
  lineItems: { quantity: Prisma.Decimal | string; unitCost: Prisma.Decimal | string }[],
): Prisma.Decimal {
  return lineItems.reduce(
    (sum, line) =>
      sum.add(new Prisma.Decimal(line.quantity).mul(new Prisma.Decimal(line.unitCost))),
...
export function assertNonEmptyLineItems(lineItems: unknown[]): void {
  if (lineItems.length === 0) {
    throw new BadRequestException('A purchase record needs at least one line item');
  }
}
